...
In this course project, we use the Weight Lifting Exercise Dataset (available at <http://groupware.les.inf.puc-rio.br/har>) to build a random forest model that predicts how well a person performs dumbell lifts.
...
Measurements from accelerometers mounted on the participants' belts, gloves, armbands and dumbbels are stored in the Weight Lifting Exercise Dataset (available at <http://groupware.les.inf.puc-rio.br/har>).
...
model
```
Note that we resort to the 5-fold cross-validation resampling technique instead of the default bootstraping in order to improve perfomance.
